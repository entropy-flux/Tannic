...
template<typename S, typename Wt, typename B, typename D>
__global__ void conv2dBiasedKernel(
...
template<typename S, typename Wt, typename B, typename D>
status launchConv2DBiasedKernel(
    const tensor_t* x, const tensor_t* w, const tensor_t* b, tensor_t* y, stream_t stream, 
    const size_t pad[2], const size_t stride[2]
) {
    if (x->rank != 4 || w->rank != 4 || y->rank != 4 || b->rank != 1) return ERROR;
    cudaStream_t cudaStream = reinterpret_cast<cudaStream_t>(stream.address);
    ...
    conv2dBiasedKernel<S, Wt, B, D><<<grid, block, 0, cudaStream>>>(
        static_cast<const S*>(x->address),
        static_cast<const Wt*>(w->address),
...
constexpr static status launchDefaultBiasedConv2DKernel(const tensor_t*, const tensor_t*, const tensor_t*, tensor_t*, stream_t, const size_t[2], const size_t[2]) {
    return UNSUPPORTED_DTYPE;
} 

using Conv2DKernel = status(*)(const tensor_t*, const tensor_t*, tensor_t*, stream_t, const size_t[2], const size_t[2]);
using Conv2DBiasedKernel = status(*)(const tensor_t*, const tensor_t*, const tensor_t*, tensor_t*, stream_t, const size_t[2], const size_t[2]);
...
constexpr auto dispatchConv2DCUDABiased = []() {
    std::array<Conv2DBiasedKernel, index(TYPES, TYPES)> table; table.fill(launchDefaultBiasedConv2DKernel);
    table[index(int8, int8)] = launchConv2DBiasedKernel<int8_t, int8_t, int8_t, int8_t>; 
    table[index(float16, float16)] = launchConv2DBiasedKernel<__half, __half, __half, __half>; 
    table[index(float32, float32)] = launchConv2DBiasedKernel<float, float, float, float>;
    table[index(float64, float64)] = launchConv2DBiasedKernel<double, double, double, double>;
    return table;
}(); 
...
status conv2d(const tensor_t* signal, const tensor_t* kernel, const tensor_t* bias,  tensor_t* dst, stream_t stream, const size_t pad[2], const size_t stride[2]) {
    return dispatchConv2DCUDABiased[index(signal->dtype, kernel->dtype)](signal, kernel, bias, dst, stream, pad, stride);
}
