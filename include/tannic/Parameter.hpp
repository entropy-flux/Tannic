...
public: 
    ...
    Tensor forward() const {
        return Tensor(float32, {2,2});
    }

public:  
    template<class Index>
    constexpr auto operator[](Index index) const {  
        return view::Slice<Parameter, Index>(*this, index);
    }

    constexpr auto operator[](view::Range index) const { 
        return view::Slice<Parameter, view::Range>(*this, index);
    }

    constexpr auto transpose(int first = -1, int second = -2) const {
        return view::Transpose<Parameter>(*this, std::make_pair<int, int>(std::move(first), std::move(second)));
    }
