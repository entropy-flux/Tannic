...
protected:
    ...
    void SetUp() override { 
        float* data = reinterpret_cast<float*>(A.buffer());
        for (int i = 0; i < 6; ++i) {
...
TEST_F(TestFunctions, Log) {
    Tensor result = log(A);
    const float* data = reinterpret_cast<const float*>(result.buffer());
...
TEST_F(TestFunctions, Abs) {
    ...
    float* original_data = reinterpret_cast<float*>(A.buffer());
    for (int i = 0; i < 6; ++i) {
